...
<head>
    ...
    <style>
        body { 
            ...
            background: linear-gradient(135deg, #e8f0fe 0%, #f0f4f8 50%, #e0e7ff 100%); 
            color: #1e293b;
            display: flex; 
        ...
        .form-container { 
            ...
            padding: 30px; 
            border-radius: 12px; 
            box-shadow: 0 8px 32px 0 rgba(100, 116, 139, 0.18); 
            width: 100%; 
            max-width: 480px; 
            position: relative;
        }
        ...
        .btn-mic:hover { background-color: #1d4ed8; }
        .btn-mic.grabando { 
        ...
        .btn-enviar:hover { background-color: #047857; }
        .alerta { 
            background-color: #d1fae5; 
            color: #047857; 
            padding: 15px; 
...
<div class="form-container">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
        ...
        <div class="input-group">
            ...
            <div class="campo-voz">
                <textarea class="campo-texto" id="nombre_completo" name="nombre_completo" rows="6" placeholder="Dicta o escribe el nombre" required></textarea>
        ...
        <div class="input-group">
            ...
            <div class="campo-voz">
                <textarea class="campo-texto" id="numero_contrato" name="numero_contrato" rows="2" placeholder="Dicta o escribe el contrato" required></textarea>
...
<script>
    ...
    async function inicializarVosk() {
        ...
        try {
            // Determinar la ruta raíz de /mvps independientemente del subdirectorio actual
            let rootPath = window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/'));
            if (rootPath.endsWith('/v-ospv') || rootPath.endsWith('/vosk-sm')) {
    ...
    function actualizarBadgeVoz(texto = null, clase = null) {
        ...
        if (cargandoModelo) {
            ...
            badge.innerHTML += ' <button type="button" onclick="desactivarVosk()" style="margin-left: 8px; background: #fee2e2; border: 1px solid #fca5a5; color: #b91c1c; border-radius: 4px; padding: 2px 6px; font-size: 9px; cursor: pointer; line-height: 1;">Desactivar</button>';
        } else {
            badge.innerText = '🎤️ Dictado por voz desactivado';
            badge.className = 'status-badge status-error';
            // Agregar botón de encendido de recursos
            badge.innerHTML += ' <button type="button" onclick="inicializarVosk()" style="margin-left: 8px; background: #dbeafe; border: 1px solid #93c5fd; color: #1d4ed8; border-radius: 4px; padding: 2px 6px; font-size: 9px; cursor: pointer; line-height: 1;">Activar</button>';
        }
    }
